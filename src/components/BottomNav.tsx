...
export default function BottomNav({ activeTab, setActiveTab }: BottomNavProps) {
  ...
  return (
    <div className="fixed bottom-6 left-6 right-6 md:hidden z-40" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <nav className="h-16 bg-white/70 dark:bg-zinc-900/80 backdrop-blur-xl border border-black/5 dark:border-white/10 flex items-center justify-around px-2 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] pb-safe">
        {tabs.map((tab) => {
          const Icon = tab.icon;
